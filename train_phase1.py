...
def train():
    ...
    # Hyperparameters - Phase 1 Optimized
    batch_size = 128
    lr = 5e-5  # INCREASED from 1e-5
    num_epochs = 20 # Reduced since validation is faster
    image_size = 224
    ...
    # Loss weights - Phase 1 Optimized
    lambda_ddpm = 1.0
    lambda_wav_ll = 3.0   # RELAXED from 10.0 to 3.0 to allow structural changes
    lambda_wav_hi = 2.0   # INCREASED from 1.0 to 2.0 to preserve details
    lambda_dan_expr = 1.6 # Kept same


    # Directories for Phase 1
    log_dir = "WaveletDiffusion_phase1/runs/exp"
    model_path = "WaveletDiffusion_phase1/model"
    out_path = "WaveletDiffusion_phase1/out"
    ...
    # Phase 1: Add OneCycleLR Scheduler
    from torch.optim.lr_scheduler import OneCycleLR
    scheduler = OneCycleLR(
        optimizer,
        max_lr=[lr, lr * 0.1], # Matching parameter groups
        epochs=num_epochs,
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for i, (img_real, expr_org, _, _) in epoch_bar:  # Bỏ valence, arousal
                ...
                # Note: Keeping clamp mild to avoid gradient kill
                pred_x0_img = torch.clamp(pred_x0_img, -1.5, 1.5)
                ...
                dan_input = torch.clamp(dan_input, 0, 1) # Ensure DAN input valid
                ...
                # Phase 1: REMOVED time thresholding. Apply DAN loss at all steps.
                # Use basic CrossEntropyLoss
                dan_expr_loss = F.cross_entropy(dan_out, expr_org)
                ...
                wav_loss = perceptual_wavelet_loss(
                    torch.clamp(pred_x0_img, -1, 1), # Clamp for VGG
                    img_real,
                    lambda_ll=lambda_wav_ll, # Optimized weight (3.0)
                    lambda_hi=lambda_wav_hi  # Optimized weight (2.0)
                )
                ...
                if i % 50 == 0:
                    ...
                    writer.add_scalar("Training/LearningRate", lr_current, step)
                        ...
                        for sample_idx in range(min(4, x_fixed.shape[0])):
                            ...
                            for emotion_id in range(len(labels)):
                                ...
                                generated, cond_params = base_model.sample(
                                    sample,
                                    emotion_tensor,
                                    num_steps=50, # Speed up validation
