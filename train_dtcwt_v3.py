...
from model_dtcwt_v3 import DirectionalWaveDiffusionModel, DTCWTWrapper, IDTCWTWrapper, AnalyticPhaseTransport
...
class TrainingWrapper(nn.Module):
    ...
    def forward(self, img_real, expr_org, drop_prob=0.0):
        ...
        with torch.no_grad():
            ...
            v_pred_mean = v_pred.abs().mean()
            v_pred_max = v_pred.abs().max()
...
def train():
    ...
    # Hyperparameters
    # ============================
    batch_size = 4          # Smaller default since DTCWT uses more memory
    lr = 4e-4
    num_epochs = 10
    image_size = 224
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for i, (img_real, expr_org, _, _) in epoch_bar:
                ...
                if i % log_every == 0:
                    ...
                    writer.add_scalar("DTCWT/VPred_mean", outputs['v_pred_mean'].mean().item(), step)
                    writer.add_scalar("DTCWT/VPred_max", outputs['v_pred_max'].max().item(), step)
